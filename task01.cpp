// Реализуйте функцию merge, которая объединяет два отсортированных контейнера в один отсортированный. Требования: сложность по времени - O(M + N). M, N размер контейнеров; сложностью по памяти - O(1).
